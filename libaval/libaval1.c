...
AvalStr_check_list(AUTOMATO * inicio,int node)
{
  AUTOMATO * aux;
  printf("node %d:\n",node);
...
/*******************************************************************/
/* void AvalStr_avaliador(AUTOMATO *, b_vector [],int[], int *,    */
/*                        int *, int)                              */
...
void AvalStr_avaliador(automatos_inicio,N,tot_req_exec,func_seq_exec,func_no_invoc,node)
...
int numero;
long seq_exec = *func_seq_exec;
long no_func_invoc = *func_no_invoc;

 /* algoritmo de avaliacao */

 if(node != EOF) /* diferente de fim de arquivo */
  {
 ...
 *func_seq_exec = seq_exec;
 *func_no_invoc = no_func_invoc;
 return;
}
