 ...
 while(h != sentinel)
    {
     ...
     if(!strcmp(h->nome,"declarat"))
		 hdeclaration = h;
     if(!strcmp(h->nome,"expressi"))
		 hexpression = h;
     if(!strcmp(h->nome,"statemen"))
		 hstatement= h;
     h = h->suc;
   }
}
      ...
      if(!strcmp(s->nome, saida.classe))
		{
		 match = VERDADEIRO;
#ifdef DEBUG
 		 if (!strcmp(saida.label,"__gconv_end_fct")) 
                      {
		 ...
		 rot_sem(s->sem);
		 if (strcmp(saida.classe,"FALHE")) /* pega o proximo simbolo so'se for fiferente de FALHE */
		  yylex();
		}
      else
	   {
