/*
// parserg1.c 	1.1		94/12/26	11:27:05
*/
 ...
 if(h1 == sentinel)
   {
	...
	error("* * Erro Fatal: Nao consegui alocar espaco para sentinel * * \r");
	h1->suc = sentinel;
 ...
 if(sym.tipo == 't' || sym.tipo == 'n' || sym.nome[0] == EMPTY)
    {
     ...
     if(a== (struct nodo *) NULL)
        error("* * Erro Fatal: Nao consegui alocar espaco para \"a\" em factor() * *\r");
	 ...
	 if(!strcmp(sym.nome,"["))
	   {
	    ...
	    if (b == (struct nodo *) NULL)
	       error("* * Erro Fatal: Nao consegui alocar espaco para \"b\" em factor() * *\r");
	    ...
	    if(!strcmp(sym.nome,"]"))
	      {pegsim();
           ...
           printf("%s\n",sym.nome);
             	...
             	else {printf("Simbolo indefinido %s\n",sym.nome);exit(1);}
	   }
	 else {printf("Simbolo indefinido %s\n",sym.nome);exit(1);}
	}
 }
