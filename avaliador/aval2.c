...
/*******************************************************************/
/* void pdu_init_aval(AUTOMATO **, b_vector *)                    */
...
void pdu_init_aval(ptr_pdu,N,test_history,descritores)
AUTOMATO ** ptr_pdu;
b_vector * N;
FILE * test_history;
FILE * descritores;
{
 ...
 /* pega o primeiro descritor ja satisfeito se existir o arquivo .his */
 ...
 while(ptr_linha != (char *) NULL)
	...
	if(test_history != (FILE *) NULL)
	  ...
	  if(pdu_aux->n_path == ac_path && ac_path !=0 )
        ...
        if(fgets(linha,MAXLINE-1,descritores) == (char *) NULL)
			 error("* * Erro Fatal: Arquivo de descritores imperfeito * *");
  ...
  do
    {
     if(fgets(linha,MAXLINE-1,file) == (char *) NULL)
       error("* * Erro Fatal: Arquivo descritores imperfeito * *");
 ...
 while(*ptr_linha != '\0')
      {
       ...
       if(!e_numero(token))
         error("* * Erro Fatal: Estava esperando ler um numero * *");
